...
class OthelloNNet():
    def __init__(self, game, args):
        ...
        self.input_scores = Input(shape=(2,))
        x_scores = Reshape((2, 1, 1))(self.input_scores)
        conv5 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(args.num_channels, 2, padding='same', use_bias=False)(x_scores)))
        scores = Dense(10, activation='softmax')(conv5)#(self.input_scores)

        h_conv4_flat = Flatten()(h_conv4)
        scores_flat = Flatten()(scores)
        concat =  Concatenate(axis=-1)([h_conv4_flat, scores_flat])
  
        s_fc1 = Dropout(args.dropout)(Activation('relu')(BatchNormalization(axis=1)(Dense(1024, use_bias=False)(concat))))  # batch_size x 1024
        ...
        self.model = Model(inputs=[self.input_boards, self.input_scores], outputs=[self.pi, self.v])
