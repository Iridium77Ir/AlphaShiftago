...
from .Shiftago import ExtremeShiftagoGame as Sf
import numpy as np
import copy
...
class shiftagoGame(Game):
    ...
    def getNextState(self, board, player, action):
        ...
        return (game.board, 1 if game.turn == True else -1, game.scores)
    ...
    def getGameEnded(self, board, player, scores):
        game = Sf()
        game.board = np.copy(board)
        game.scores = copy.copy(scores)
        game.turn = True if player == 1 else False
        return game.checkEnd()#game.checkEnd() * player

    def getCanonicalForm(self, board, player, scores):
        newScores = copy.copy(scores)
        if player == -1:
            temp = newScores[0]
            newScores[0] = newScores[1]
            newScores[1] = temp
        return (player*board, newScores)
    ...
    def stringRepresentation(self, board, scores):
        b = np.copy(board)
        s = copy.copy(scores)
        arr = np.concatenate((b.flatten(),np.asarray(s).flatten()), axis=None)
        return arr.tostring()
